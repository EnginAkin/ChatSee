...
public class UserServiceBean implements UserService{
	...
	public void add(User user) throws ExistUserEmailException {
		System.out.println("add fonksiyonuna gelindi . ");
	...
	@Override
	public void saveChangeUserDetail(User user) {
		System.out.println("id : "+user.getId());
		Optional<User> usr=this.userDAO.findByEmail(user.getEmail());
		user.setPassword(usr.get().getPassword());
		this.userDAO.save(user);
	}
	...
	public UserInfo getUserInfo(User user) {
		...
		UserInfo info = new UserInfo(user.getId(), user.getName(), user.getSurname(), getFriends(user.getId()), user.getMainLanguage(),user.getWantToLearnLanguage(), user.getEmail(),user.getCreated());
		return info;
	}
