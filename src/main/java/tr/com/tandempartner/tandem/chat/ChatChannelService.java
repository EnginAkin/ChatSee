...
public class ChatChannelService {
	...
	public ChatChannelForUsers add(long userid1,long userid2) {
		...
		ChatChannelForUsers chat = new ChatChannelForUsers((long)(channels.size()+1), userid1, userid2, true, false,false);
		...
		return chat;
			...
			if(chatChannelExist.isUser1_accept() && chatChannelExist.isUser2_accept()){
				chatChannelExist.setUsed(true);
			}
	...
	public boolean isExistChannel(long id1,long id2) {
		for (ChatChannelForUsers chatChannelForUsers : channels) {
			if(!chatChannelForUsers.isUsed() && chatChannelForUsers.isExist(id1, id2) ) {
				return true;
			}
		}
		return false;
